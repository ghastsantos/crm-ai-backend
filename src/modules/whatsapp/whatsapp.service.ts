...
async function moveDealAutomatically(
  ...
  await createPipelineLog({
    ...
    description: `Moveu a negociacao "${updated.title}" de "${deal.pipelineColumn.title}" para "${targetColumn.title}" via WhatsApp.`,
...
async function applyCrmAction(input: {
  ...
  if (!input.existingDeal) {
    ...
    await createPipelineLog({
      ...
      description: `Criou a negociacao "${deal.title}" na coluna "${targetColumn.title}" via WhatsApp.`,
      toColumnId: targetColumn.id,
      toColumnName: targetColumn.title,
      metadata: {
        dealId: deal.id,
  ...
  await createPipelineLog({
    ...
    description: `Atualizou a negociacao "${updated.title}" via WhatsApp.`,
